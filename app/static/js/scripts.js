...
function displayQuestion() {
    ...
    if (currentQuestionIndex < questions[currentGrade].length) {
        ...
        questionArea.innerHTML += `<br><button onclick="sendDataToPython2(score/questions[currentGrade].length, currentGrade)">Submit</button>`
    }
}
...
function sendNumberToPython(score) {
    const numberToSend = score; // Replace with the number you want to send
    fetch('/getScore', {
        method: 'POST',
        headers: {
